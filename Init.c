...
//    temp[i] = MU/R*(ADIABATICINDEX-1.0)*energ[l]/dens[l];
  ...
  for (i=0; i<nr; i++){
    for (j=0; j<ns; j++){
      l = i*ns+j;
        dens[l] = -mdot/3./PI/(ALPHAVISCOSITY* ADIABATICINDEX * temp[i] * pow(Rmed[i], 1.5));
  ...
  real r, temp, mdot;
{
  ...
  sigma = mdot/ALPHAVISCOSITY/3/PI*pow(r,-1.5)/temp/ADIABATICINDEX;
...
/* Convert 3D volume density into g.cm^-3 */
  H = sqrt(ADIABATICINDEX*temp) / pow(r,-1.5);
  rho3D = 0.5*sigma/H;  // 3D density = sigma / 2H, in code units
